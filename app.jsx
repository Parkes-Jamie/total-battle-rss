...
function App() {
  ...
  const saveEdit = async () => {
    ...
    await supabase.from('weekly_totals').upsert(
      { player_id: editId, week_id: weekId, wood: t.Wood, stone: t.Stone, iron: t.Iron, food: t.Food, silver: t.Silver },
  ...
  const btn = (v = 'default') => ({ borderRadius: 3, padding: '5px 12px', fontSize: 10, fontFamily: 'inherit', letterSpacing: '.08em', cursor: 'pointer', textTransform: 'uppercase', border: '1px solid', transition: 'all .15s', ...(v === 'active' ? { background: '#e8a020', color: '#0d0e10', borderColor: '#e8a020' } : v === 'primary' ? { background: '#e8a020', color: '#0d0e10', borderColor: '#e8a020', fontWeight: 700 } : v === 'danger' ? { background: '#2a0a0a', color: '#f87171', borderColor: '#7f1d1d' } : { background: '#1a1a1e', color: '#6a5a3a', borderColor: '#2a2a30' }) });
  ...
  if (loading) return <div style={{ minHeight: '100vh', background: '#0d0e10', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#e8a020', fontFamily: 'Courier New', letterSpacing: '.1em', fontSize: 13 }}>LOADING...</div>;
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0d0e10', color: '#d4c9b8', fontFamily: "'Courier New',monospace", fontSize: 13, paddingBottom: 40 }}>
      ...
      <div style={{ background: 'linear-gradient(180deg,#1a1200,#0d0e10)', borderBottom: '1px solid #e8a02028', padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50 }}>
        <div>
          <div style={{ fontSize: 15, fontWeight: 700, letterSpacing: '.14em', textTransform: 'uppercase', color: '#e8a020' }}>
            ⚔ RSS Tracker {admin && <span style={{ fontSize: 10, color: '#ef4444' }}>ADMIN</span>}
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {!admin && <div style={{ fontSize: 9, color: '#3a2a0a' }}>READ ONLY</div>}
            ...
            {DAYS.map((d, i) => {
              ...
              return (
                <div key={d} onClick={() => admin && !isFuture && toggleSubmit(d, r)}
                  style={{ height: 22, borderRadius: 3, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, cursor: admin && !isFuture ? 'pointer' : 'default', background: done ? RES_COL[r] + '30' : isFuture ? '#0d0e10' : '#1a1a1e', border: `1px solid ${done ? RES_COL[r] + '80' : i === today ? '#3a3020' : '#1e1e22'}`, opacity: isFuture ? 0.2 : 1 }}>
                  {done ? '✓' : i < today && !done ? '✗' : '·'}
                </div>
              );
            })}
      ...
      <div style={{ display: 'flex', gap: 8, padding: '8px 18px', background: '#111214', borderBottom: '1px solid #1a1a1e', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <span style={{ marginLeft: 'auto', fontSize: 9, color: '#3a2a0a' }}>{players.length} PLAYERS</span>
      </div>
